...
export default function HouseWorkerListView() {
  ...
  useEffect(() => {
    const savedPage = parseInt(sessionStorage.getItem("worker-page"), 10) - 1;
    const savedRowsPerPage = parseInt(sessionStorage.getItem("worker-rows"), 10);
  ...
  const handlePageChange = (event, newPage) => {
    ...
    sessionStorage.setItem("worker-page", newPage + 1);
  };

  const handleRowsPerPageChange = (event) => {
    const newRowsPerPage = parseInt(event.target.value, 10);
    table.onChangeRowsPerPage(event, newRowsPerPage);
    sessionStorage.setItem("worker-rows", newRowsPerPage);
    table.onChangePage(null, 0);
    sessionStorage.setItem("worker-page", 1);
  };
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    sessionStorage.removeItem("worker-page");
