...
import { sumIncomeByYear } from 'src/utils/utils';
...
export default function EcommerceYearlySales({ title, subheader, chart, ...other }) {
  ...
  const [incomeYear, setIncomeYear] = useState(sumIncomeByYear(series, '2024'));
  ...
  const handleChangeSeries = useCallback(
    (newValue) => {
      ...
      setIncomeYear(sumIncomeByYear(series, newValue));
    },
    [popover, series]
  );
  ...
  return (
    <>
      <Card {...other}>
        <CardHeader
          title={title}
          subheader={`Thu nhập: ${fCurrency(incomeYear)}`}
          action={
            <ButtonBase
